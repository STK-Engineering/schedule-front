...
export default function Request() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const res = await api.get("/leaves/pending-requests");
      ...
      const mapped = list.map((e) => ({
        ...
        type: e.leaveType ?? "-",
        date: e.startDate ?? "",
        days: e.usedDay != null ? `${e.usedDay}일` : "-",
          ...
          {data.length === 0 ? (
            ...
            data.map((item) => (
              <Item
                ...
                item={item}
                onPress={() =>
                  navigation.navigate("LeaveRequestContent", { ...item })
                }
...
function Item({ item, onPress, onApprove, onReject }) {
  const title = `${displayLeaveType(item.type)}(${item.name || "-"})`;
  return (
